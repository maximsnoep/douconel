...
mod tests {
    ...
    #[test]
    fn from_quad_obj() {
        let douconel = Douconel::<EmbeddedVertex, (), ()>::from_obj("assets/cube.obj");
        assert!(douconel.is_ok());
        if let Ok((douconel, _, _)) = douconel {
            ...
            assert!(g.edge_count() == 4 * 6);
        }
    }

    #[test]
    fn from_quad_noisy_obj() {
        let douconel = Douconel::<EmbeddedVertex, (), ()>::from_obj("assets/cube_noisy.obj");
        assert!(douconel.is_ok());
        if let Ok((douconel, _, _)) = douconel {
            assert!(douconel.nr_verts() == 8);
            assert!(douconel.nr_edges() == 4 * 6);
