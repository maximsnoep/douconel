...
impl<VertID: Key, V: Default, EdgeID: Key, E: Default, FaceID: Key, F: Default> Douconel<VertID, V, EdgeID, E, FaceID, F> {
    ...
    #[must_use]
    pub fn graph_modified<W, T>(
        &self,
        filter_verts: &HashSet<VertID>,
        filter_edges: &HashSet<EdgeID>,
        weight_function: W,
    ) -> (Csr<(), T, Directed, u32>, BiHashMap<VertID, u32>)
    where
        W: Fn(EdgeID) -> T,
        T: Clone,
    {
        let mut key_to_int = BiHashMap::new();
        for (i, id) in self.verts.keys().enumerate() {
            key_to_int.insert(id, i as u32);
        }
        ...
        let sorted_edges = self
            ...
            .map(|id| {
                ...
                (
                    key_to_int.get_by_left(&u).unwrap().to_owned(),
                    key_to_int.get_by_left(&v).unwrap().to_owned(),
                    weight_function(id),
                )
            })
    ...
    pub fn graph_with_weights<W, T>(&self, weight_function: W) -> (Csr<(), T, Directed, u32>, BiHashMap<VertID, u32>)
    where
        W: Fn(EdgeID) -> T,
        T: Clone,
    {
        self.graph_modified(&HashSet::new(), &HashSet::new(), weight_function)
    }

    #[must_use]
    pub fn graph<W, T>(&self) -> (Csr<(), (), Directed, u32>, BiHashMap<VertID, u32>)
    where
        W: Fn(EdgeID) -> T,
        T: Clone,
    {
        self.graph_modified(&HashSet::new(), &HashSet::new(), |_| ())
    }
    ...
    // pub fn graph_filtered<W, T>(&self, filter_verts: &HashSet<VertID>, filter_edges: &HashSet<EdgeID>, weight_function: W) -> DiGraphMap<VertID, T>
    // where
    //     W: Fn(EdgeID) -> T,
    // {
...
// impl<V: HasPosition, E, F: HasNormal> Douconel<V, E, F> {
//     // To petgraph: edge graph with <>DWAJD@$@!KM# edge weights
//     pub fn special_orientation_graph(&self, direction: Vec3, gamma: f32) -> DiGraphMap<EdgeID, f32> {
